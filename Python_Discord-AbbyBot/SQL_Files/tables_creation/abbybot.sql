...
CREATE TABLE IF NOT EXISTS `abbybot`.`mention_counter` (
  ...
  CONSTRAINT `fk_user_server`
    ...
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'This table is in charge of storing the mentions of each independent user, if they are on several servers, the number of times and the time where they made the last mention.';
