...
class SetBirthDayChannel(commands.Cog):
    ...
    @app_commands.command(name="set_birthday_channel", description="Assign a channel so AbbyBot can celebrate users' birthdays on your server.")
    async def command_name(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        # Confirm to user
        if current_language_id == 1:
            await interaction.response.send_message(f"Birthday channel has been set to {channel.mention}. AbbyBot will greet users on that channel when they have a birthday!", ephemeral=True)
        elif current_language_id == 2:
            await interaction.response.send_message(f"El canal de cumpleaños se ha configurado en {channel.mention}. AbbyBot saludará a los usuarios de ese canal cuando cumplan años!", ephemeral=True)
